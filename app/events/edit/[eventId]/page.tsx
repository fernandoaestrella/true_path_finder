...
function EditEventContent() {
  ...
  const secondsToConfig = (totalSeconds: number): PhaseConfig => ({
    ...
    const loadEvent = async () => {
      try {
        ...
        if (eventDoc.exists()) {
          ...
          // Format start time for input (YYYY-MM-DDThh:mm)
          ...
          setFormData({
            ...
            links: data.links && data.links.length > 0 ? data.links : [{ title: '', url: '' }],
            arrival: secondsToConfig(data.phases.arrival.durationSeconds),
            practice: secondsToConfig(data.phases.practice.durationSeconds),
            close: secondsToConfig(data.phases.close.durationSeconds),
            startTime: localISOTime,
            maxPerBatch: data.maxPerBatch,
            repeatability: data.repeatability || { type: 'none', interval: 1, daysOfWeek: [] },
          });
        } else {
          ...
          <Card className="mb-6">
            ...
            <div className="mb-6">
              <label className="flex items-center text-lg font-medium text-[var(--text-primary)] mb-2">
                ...
                <InfoTooltip content="Batches are limited to 21 participants to ensure meaningful connection." />
              </label>
              <Input
                 type="number"
